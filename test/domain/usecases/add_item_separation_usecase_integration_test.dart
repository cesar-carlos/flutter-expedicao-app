  ...
  group('AddItemSeparationUseCase Integration Tests', () {
    ...
    group('Teste de Sucesso', () {
      test('deve adicionar item à separação com sucesso', () async {
        ...
        final separateBeforeSuccess = await separateItemRepository.select(
          QueryBuilder()
              .equals('CodEmpresa', params.codEmpresa)
              .equals('CodSepararEstoque', params.codSepararEstoque)
              .equals('CodProduto', params.codProduto),
        );
        expect(separateBeforeSuccess, isNotEmpty, reason: 'Item de teste deve existir no repositório');
        final quantidadeSeparacaoAntes = separateBeforeSuccess.first.quantidadeSeparacao;
        ...
        expect(
          ...
          equals(quantidadeSeparacaoAntes + params.quantidade),
          reason:
              'Quantidade total separada deve ser a já acumulada ($quantidadeSeparacaoAntes) '
              'mais a desta operação (${params.quantidade})',
        );
    ...
    group('Testes de Múltiplas Separações', () {
      test('deve adicionar múltiplos itens e atualizar quantidade corretamente', () async {
        ...
        expect(
          result2.isSuccess(),
          isTrue,
          reason: 'Segunda separação deveria ter sucesso; falha: ${result2.exceptionOrNull()}',
        );
