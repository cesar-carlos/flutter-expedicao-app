  ...
  group('AddItemSeparationUseCase Integration Tests', () {
    ...
    setUp(() async {
      ...
      try {
        ...
        final insertResult = await separateItemRepository.insert(
          testSeparateItem,
        );
        ...
        await UseCaseIntegrationTestBase.waitForOperation(
        ...
        if (e.toString().contains('PRIMARY KEY constraint')) {
          ...
          UseCaseIntegrationTestBase.logUnexpectedError(
            'Preparação dos dados',
            e,
          );
          rethrow;
    ...
    group('Teste de Sucesso', () {
      test(
        'deve adicionar item à separação com sucesso',
        () async {
          ...
          expect(
            success.codProduto,
            equals(params.codProduto),
            reason: 'Código do produto deve coincidir',
          );
          expect(
            success.addedQuantity,
            equals(params.quantidade),
            reason: 'Quantidade adicionada deve coincidir',
          );
          expect(
            success.createdSeparationItem.sessionId,
            equals(params.sessionId),
            reason: 'SessionId deve coincidir',
          );
          expect(
          ...
          expect(
            success.separatorName,
            equals(params.nomeSeparador),
            reason: 'Nome do separador deve coincidir',
          );
          expect(
            success.hasRemainingQuantity,
            isTrue,
            reason: 'Deve haver quantidade restante',
          );

          UseCaseIntegrationTestBase.logTestSuccess(
            'Adição de item',
            details: success.operationSummary,
          );
        },
    ...
    group('Testes de Regras de Negócio', () {
      test(
        'deve falhar quando quantidade solicitada excede disponível',
        () async {
          UseCaseIntegrationTestBase.logTestStart(
            'Validação de quantidade excessiva',
          );

          // Arrange
          final params = createTestAddItemSeparationParamsWithExcessiveQuantity(
            sessionId,
          );
          ...
          expect(
            result.isSuccess(),
            isFalse,
            reason: 'UseCase deveria falhar com quantidade excessiva',
          );
          ...
          expect(
            failure!.type,
            equals(AddItemSeparationFailureType.insufficientQuantity),
          );
    ...
    group('Testes de Múltiplas Separações', () {
      test(
        'deve adicionar múltiplos itens e atualizar quantidade corretamente',
        () async {
          ...
          final initialSeparateItems = await separateItemRepository.select(
            QueryBuilder()
                .equals('CodEmpresa', 1)
                .equals('CodSepararEstoque', 999999)
                .equals('CodProduto', 1),
          );
          ...
          final params1 = createTestAddItemSeparationParamsForMultiple1(
            sessionId,
          );
          final params2 = createTestAddItemSeparationParamsForMultiple2(
            sessionId,
          );
          ...
          expect(
            result1.isSuccess(),
            isTrue,
            reason: 'Primeira separação deveria ter sucesso',
          );
          expect(
            result2.isSuccess(),
            isTrue,
            reason: 'Segunda separação deveria ter sucesso',
          );
          ...
          final expectedTotal =
              initialQuantitySeparated +
              success1.addedQuantity +
              success2.addedQuantity;
          expect(
