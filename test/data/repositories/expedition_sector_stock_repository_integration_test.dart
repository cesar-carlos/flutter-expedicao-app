...
void main() {
  group('ExpeditionSectorStockRepositoryImpl Integration Tests', () {
    ...
    group('INSERT Test', () {
      test(
        'deve inserir um novo setor de estoque e verificar se foi inserido',
        () async {
          ...
          expect(
            insertResult,
            isNotEmpty,
            reason: 'O resultado da inserção não deve estar vazio',
          );
          expect(
            insertResult.first.descricao,
            newSectorStock.descricao,
            reason:
                'A descrição do setor inserido deve corresponder ao enviado',
          );
          expect(
            insertResult.first.ativoCode,
            'S',
            reason: 'O setor deve ser inserido como ativo (S)',
          );
          expect(
            insertResult.first.ativoDescription,
            'Sim',
            reason: 'A descrição do status deve ser "Sim"',
          );
    ...
    group('UPDATE Test', () {
      test(
        'deve atualizar o setor de estoque inserido anteriormente',
        () async {
          // Cria uma versão atualizada do setor de estoque
          final updatedSectorStock = createUpdatedTestSectorStock(
            insertedSectorStock,
          );
          ...
          expect(
            updateResult,
            isNotEmpty,
            reason: 'O resultado da atualização não deve estar vazio',
          );
          expect(
          ...
          expect(
            updateResult.first.ativoCode,
            'N',
            reason: 'O status deve ter sido alterado para inativo (N)',
          );
    ...
    group('DELETE Test', () {
      test(
        'deve deletar o setor de estoque inserido anteriormente',
        () async {
          ...
          expect(
            deleteResult,
            isNotEmpty,
            reason: 'O resultado da deleção não deve estar vazio',
          );
          expect(
          ...
          expect(
            deleteResult.first.ativoCode,
            isA<String>(),
            reason: 'Deve retornar um código de status válido',
          );
