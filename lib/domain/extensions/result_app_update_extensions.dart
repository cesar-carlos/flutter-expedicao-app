...
extension ResultAppUpdateExtensions<T extends Object> on Result<T> {
  Result<T> mapFailureToAppUpdate(
    AppUpdateFailure Function(Exception) mapper,
  ) {
    return fold(
      (s) => Success<T, Exception>(s),
      (err) => Failure<T, Exception>(mapper(err)),
    );
  }
}

extension ResultVoidAppUpdateExtensions on Result<void> {
  Result<void> mapFailureToAppUpdate(
    AppUpdateFailure Function(Exception) mapper,
  ) {
    return fold(
